# Terms of Service

**Last updated: January 20, 2025**

These Terms of Service ("Terms") govern your use of the BabyFeed mobile application ("App", "Service", "BabyFeed").

**By downloading, installing, or using BabyFeed, you agree to these Terms. If you do not agree, do not use the App.**

---

## 1. SERVICE PROVIDER

**Mickael Akim Gauthier**  
Individual Entrepreneur  
SIREN: 879 819 480  
SIRET: 879 819 480 00044  
Bureau 326, 78 Avenue des Champs-Élysées  
75008 Paris, France  
Email: amgiosdev@gmail.com
Legal contact: amgiosdev@gmail.com

---

## 2. ACCEPTANCE OF TERMS

### 2.1 Agreement

By using BabyFeed, you acknowledge that:

- You have read, understood, and agree to these Terms
- You have read and agree to our [Privacy Policy]([https://github.com/AmgiOS/babyfeed-privacy/blob/main/privacy.md]))
- You are at least 13 years old (or legal age in your jurisdiction)
- You have the legal capacity to enter into a binding agreement

### 2.2 Parental Use

- BabyFeed is designed for parents/guardians to track their baby's feeding
- You must be 13+ years old to use the App
- You are responsible for all data entered about your child
- You have full parental authority over the child's data

### 2.3 Changes to Terms

We reserve the right to modify these Terms at any time:

- **Notification**: In-app message and/or email (if provided)
- **Effective date**: 30 days after notification
- **Continued use**: Constitutes acceptance of new Terms
- **Rejection**: You may stop using the App before changes take effect

---

## 3. LICENSE AND USAGE

### 3.1 License Grant

We grant you a limited, non-exclusive, non-transferable, revocable license to:

- Install and use BabyFeed on your personal iOS device
- Use the App for personal, non-commercial purposes
- Track feeding data for your baby/babies

### 3.2 Restrictions

You agree NOT to:

- ❌ Modify, reverse engineer, or decompile the App
- ❌ Remove copyright notices or proprietary markings
- ❌ Use the App for commercial purposes without authorization
- ❌ Resell, rent, lease, or distribute the App
- ❌ Create derivative works based on the App
- ❌ Use automated systems (bots, scrapers) with the App
- ❌ Attempt to access source code or backend systems
- ❌ Interfere with the App's security features
- ❌ Use the App for illegal purposes

### 3.3 Ownership

All intellectual property rights in BabyFeed remain with Mickael Akim Gauthier:

- App design, code, graphics, logos
- Trademarks: "BabyFeed", logo, brand elements
- Algorithms, features, user interface
- Documentation and support materials

**Your data**: You retain all rights to the data you enter (baby info, bottle records, notes).

---

## 4. APP FEATURES AND FUNCTIONALITY

### 4.1 Core Features (Free Version)

BabyFeed provides:

- Unlimited bottle tracking
- Baby profile management
- Statistics and charts
- Daily/weekly/monthly averages
- Local notifications (optional)
- Data export (PDF/CSV)
- iCloud backup (optional)
- Dark mode support

### 4.2 Premium Features (If Applicable)

Premium features may include:

- Multiple baby profiles
- Advanced statistics
- Custom themes
- Priority support
- Early access to new features

**Pricing**: Displayed in the App Store (subject to change)  
**Billing**: Through Apple App Store (Apple's terms apply)  
**Refunds**: Subject to Apple's refund policy

### 4.3 Feature Changes

We reserve the right to:

- Add, modify, or remove features at any time
- Discontinue the App (with 30 days notice)
- Change pricing for new purchases (existing purchases honored)

**No guarantee**: Features are provided "as is" without warranty of continuous availability.

---

## 5. USER RESPONSIBILITIES

### 5.1 Accurate Information

You are responsible for:

- Entering accurate bottle data
- Verifying information before making decisions
- Updating baby information as needed
- Maintaining device security (passcode, Face ID)

### 5.2 Medical Disclaimer

⚠️ **IMPORTANT**: BabyFeed is a tracking tool, NOT medical advice:

- **Not a medical device**: Not FDA approved
- **Not diagnostic**: Cannot diagnose health conditions
- **Not prescriptive**: Cannot recommend treatments
- **Consult professionals**: Always consult your pediatrician for medical concerns
- **Emergency**: Call emergency services (911, 112, etc.) for urgent issues

**We are not liable for any health decisions based on App data.**

### 5.3 Data Backup

You are responsible for:

- Backing up your data (iCloud or export)
- Data loss prevention
- Device security (lost/stolen device)

**We cannot recover data from deleted apps or reset devices.**

---

## 6. PAYMENTS AND SUBSCRIPTIONS

### 6.1 In-App Purchases

If you purchase premium features:

- **Platform**: Apple App Store (Apple's terms apply)
- **Payment**: Charged to your Apple ID account
- **Confirmation**: At time of purchase
- **Auto-renewal**: May apply (can be disabled in iOS Settings)

### 6.2 Subscriptions (If Applicable)

- **Billing cycle**: Monthly or Annual (as selected)
- **Auto-renewal**: Unless cancelled 24h before renewal
- **Cancel**: iOS Settings > [Your Name] > Subscriptions
- **Price changes**: Notified 30 days in advance (must re-consent)

### 6.3 Refunds

- **Managed by**: Apple App Store
- **Request**: Apple Support or App Store > Account > Purchase History
- **Our policy**: No direct refunds (Apple handles all transactions)
- **Exceptions**: May consider case-by-case for technical issues

**Note**: Free app = no payment terms apply.

---

## 7. DATA AND PRIVACY

### 7.1 Data Storage

- **Local only**: All data stored on your device
- **No servers**: No cloud backend (except optional iCloud)
- **Encryption**: iOS native encryption (AES-256)
- **Your control**: Delete anytime in Settings

### 7.2 Privacy Policy

Your privacy is governed by our [Privacy Policy]([(https://github.com/AmgiOS/babyfeed-privacy/blob/main/privacy.md)]):

- GDPR compliant (EU)
- CCPA compliant (California)
- COPPA compliant (US)
- No tracking, no ads, no data sales

### 7.3 Data Deletion

To delete all data:

- **In-app**: Settings > Delete All Data
- **Complete**: Uninstall the App
- **iCloud**: Disable in iOS Settings > iCloud

**Deletion is immediate and irreversible.**

---

## 8. DISCLAIMERS AND LIMITATIONS

### 8.1 "AS IS" Provision

BabyFeed is provided "AS IS" and "AS AVAILABLE" without warranties:

- ❌ No warranty of uninterrupted service
- ❌ No warranty of error-free operation
- ❌ No warranty of fitness for particular purpose
- ❌ No warranty of merchantability

### 8.2 Accuracy

We strive for accuracy but cannot guarantee:

- 100% bug-free code
- Compatibility with all iOS versions/devices
- Accuracy of calculations (verify important data)
- Availability at all times

### 8.3 Third-Party Services

- **Apple App Store**: Subject to Apple's terms
- **iOS**: Subject to Apple's operating system
- **iCloud**: Subject to Apple's iCloud terms

**We are not responsible for third-party service failures.**

---

## 9. LIABILITY LIMITATIONS

### 9.1 Limitation of Liability

**To the maximum extent permitted by law**, we are NOT liable for:

- ❌ Direct, indirect, incidental, or consequential damages
- ❌ Loss of data, profits, or business opportunities
- ❌ Medical decisions based on App data
- ❌ Errors in calculations or statistics
- ❌ Device malfunction or data loss
- ❌ Third-party actions (Apple, iOS, hackers)

**Maximum liability**: Amount paid for the App (if any) in the last 12 months.

### 9.2 Exceptions

Liability limitations do NOT apply to:

- Death or personal injury caused by our negligence
- Fraud or fraudulent misrepresentation
- Violations of consumer protection laws (where not waivable)
- Other liabilities that cannot be limited by law

### 9.3 Indemnification

You agree to indemnify and hold harmless Mickael Akim Gauthier from:

- Claims arising from your use of the App
- Violations of these Terms
- Infringement of third-party rights
- Illegal use of the App

---

## 10. TERMINATION

### 10.1 By You

You may terminate at any time by:

- Uninstalling the App
- Deleting your data
- Stopping use of the Service

### 10.2 By Us

We may terminate or suspend your access if:

- You violate these Terms
- You use the App for illegal purposes
- You attempt to harm the App or other users
- Required by law or legal process

### 10.3 Effects of Termination

Upon termination:

- Your license to use BabyFeed ends immediately
- Delete the App from your device
- All data remains local (you control deletion)
- No refunds for unused subscription periods (Apple policy)

**Survival**: Sections 5 (Responsibilities), 9 (Liability), 11 (Disputes), 13 (General) survive termination.

---

## 11. DISPUTE RESOLUTION

### 11.1 Governing Law

These Terms are governed by:

- **EU users**: Laws of France and EU regulations
- **US users**: Laws of the State of California, USA
- **Other**: Laws of France (international conventions apply)

### 11.2 Jurisdiction

Disputes shall be resolved in:

- **EU users**: Courts of Paris, France (or your local courts under EU law)
- **US users**: Courts of Los Angeles County, California
- **Other**: Courts of Paris, France

### 11.3 Informal Resolution

Before filing a lawsuit, you agree to:

1. Contact us at amgiosdev@gmail.com
2. Describe the dispute in detail
3. Allow 60 days for good-faith resolution
4. Participate in mediation if requested

### 11.4 Arbitration (Optional)

You may agree to binding arbitration:

- **Rules**: International Chamber of Commerce (ICC)
- **Language**: English or French
- **Location**: Paris, France (or remote)
- **Costs**: Shared equally unless otherwise decided

**Opt-out**: Email amgiosdev@gmail.com within 30 days of first use.

### 11.5 Class Action Waiver

You agree to resolve disputes individually, NOT as:

- Class action
- Class arbitration
- Collective action
- Representative action

**Exception**: Where prohibited by law (EU, some US states).

---

## 12. COMPLIANCE AND LEGAL

### 12.1 Export Controls

You agree NOT to:

- Export the App to embargoed countries
- Use the App in violation of export laws
- Provide the App to prohibited entities

**Compliance**: You are responsible for local laws in your jurisdiction.

### 12.2 GDPR (EU Users)

If you are in the EU:

- See our [Privacy Policy](https://github.com/AmgiOS/babyfeed-privacy/blob/main/privacy.md) for GDPR rights
- You have the right to lodge complaints with your Data Protection Authority
- Data processing is based on consent and legitimate interest

### 12.3 CCPA (California Users)

If you are in California:

- You have the right to know, delete, and opt-out
- We do NOT sell your personal information
- Contact amgiosdev@gmail.com to exercise rights

### 12.4 COPPA (US Users)

- App is designed for parents (13+), not children
- No data collected directly from children under 13
- Parents control all data entry

---

## 13. GENERAL PROVISIONS

### 13.1 Entire Agreement

These Terms, together with our Privacy Policy, constitute the entire agreement between you and BabyFeed.

### 13.2 Severability

If any provision is found invalid:

- The invalid provision is severed
- Remaining provisions remain in effect
- Invalid provision replaced with valid equivalent (if possible)

### 13.3 No Waiver

Failure to enforce any provision does NOT waive our right to enforce it later.

### 13.4 Assignment

- **You**: Cannot transfer your rights under these Terms
- **Us**: May assign these Terms to affiliates or acquirers (you'll be notified)

### 13.5 Force Majeure

We are not liable for delays/failures due to:

- Natural disasters (earthquake, flood, fire)
- War, terrorism, civil unrest
- Government actions, laws, regulations
- Internet/telecommunications failures
- Other events beyond our reasonable control

### 13.6 Contact Information

For questions about these Terms:

- **General**: amgiosdev@gmail.com
- **Legal**: amgiosdev@gmail.com
- **Privacy**: amgiosdev@gmail.com

**Response time**: Within 5 business days

---

## 14. LANGUAGE AND TRANSLATIONS

### 14.1 Available Languages

These Terms are available in:

- English (reference version)
- French
- Spanish
- German
- Italian
- Portuguese

### 14.2 Discrepancies

In case of conflicts between translations, the **English version prevails**.

---

## 15. APPLE APP STORE TERMS

### 15.1 Acknowledgment

You acknowledge that:

- These Terms are between you and Mickael Akim Gauthier, NOT Apple
- Apple is not responsible for BabyFeed or its content
- Apple has no obligation to provide support
- Apple is not liable for any claims related to BabyFeed

### 15.2 Third-Party Beneficiary

Apple is a third-party beneficiary of these Terms and may enforce them.

### 15.3 Additional Requirements

If you downloaded BabyFeed from the Apple App Store:

- You must comply with Apple's [App Store Terms](https://www.apple.com/legal/internet-services/itunes/dev/stdeula/)
- You represent that you are not in an embargoed country
- You are not on a US Government prohibited parties list

---

## 16. UPDATES AND MODIFICATIONS

### 16.1 App Updates

We may release updates to:

- Fix bugs and improve performance
- Add new features
- Ensure iOS compatibility
- Address security issues

**Updates**: Automatic (if enabled) or manual via App Store  
**Requirement**: May require latest version for full functionality

### 16.2 Terms Updates

Changes to these Terms:

- **Notification**: In-app message (+ email if provided)
- **Effective date**: 30 days after notification
- **Material changes**: May require re-acceptance
- **Version history**: Available on request

---

## SUMMARY (TL;DR)

✅ **Use BabyFeed responsibly for tracking your baby's feeding**  
📱 **Personal, non-commercial use only**  
⚠️ **Not medical advice - always consult your pediatrician**  
🔒 **Your data stays on your device**  
💰 **Free app (premium features optional)**  
🚫 **No warranties - "as is" provision**  
⚖️ **Disputes resolved in Paris, France (or your local courts)**  
📧 **Questions? amgiosdev@gmail.com**  

**By using BabyFeed, you agree to these Terms.**

---

© 2025 BabyFeed - Mickael Akim Gauthier  
All rights reserved.

**Last updated: January 20, 2025**  
**Version: 1.0**

---

**Related Documents**:
- [Privacy Policy](https://github.com/AmgiOS/babyfeed-privacy/blob/main/privacy.md)
- [Support](mailto:amgiosdev@gmail.com)
- [Report Issues](mailto:amgiosdev@gmail.com)
